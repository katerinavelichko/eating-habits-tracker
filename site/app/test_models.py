...
@pytest.fixture(scope='module')
def new_user():
    user = Users(name='Test User', email='test_user@example.com', password='test_password')
...
def test_add_diary_product(user_info, new_diary_product):
    ...
    prdct = Diary.query.filter_by(user_id=user_info.id).order_by(Diary.id.desc()).first()
...
def test_add_post(new_post):
    ...
    pst = Posts.query.filter_by(user_id=new_post['user_id']).order_by(Posts.id.desc()).first()
...
def test_add_comment(new_comment):
    ...
    cmnt = Comment.query.filter_by(post_id=new_comment['post_id']).order_by(Comment.id.desc()).first()
